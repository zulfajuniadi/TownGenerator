...
namespace Town
{
    ...
    public class TownArea : Area
    {
        ...
        private IEnumerable<Polygon> CreateAlleys (Polygon block, float minArea, float gridChaos, float sizeChaos, Func<Polygon, float> emptyProbabilityFunc, bool split, int levels = 0)
        {
            ...
            {
                if (half.Area () < minArea * Math.Pow (2, 4 * sizeChaos * (random.value - 0.5f)) || levels > 5)
                {
                ...
                {
                    buildings.AddRange (CreateAlleys (half, minArea, gridChaos, sizeChaos, emptyProbabilityFunc, half.Area () > minArea / (random.value * random.value), levels + 1));
                }
            }
